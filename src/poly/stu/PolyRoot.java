...
public class PolyRoot {
    ...
    private static double newtonsMethod(int[] poly, double x0, int iter) {
        double x1 = x0 - (PolyEval.evaluate(poly, x0)/PolyEval.evaluate(PolyDerive.computeDerivative(poly), x0)); // Our next x value to test
        if (iter >= MAX_ITERATIONS || (Math.abs((x1-x0)/x0) <= EPSILON)) {
     // If (we've got to far or (as a percentage relative to the whole distance, if our distance on either side of x0 is less than our epsilon)) {
            return x1; // stop trying
        } else {
            iter++; // confirm we've gone one level lower
            return newtonsMethod(poly, x1, iter); // calculate again
        }
    }
}
